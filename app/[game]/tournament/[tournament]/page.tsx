...
function getMatchResultText(
  ...
  const leftResult = match.leftResult?.trim().toUpperCase()
  const rightResult = match.rightResult?.trim().toUpperCase()

  if (leftResult === "W") {
    return `${match.leftPlayer || "TBD"} won ${leftScore}–${rightScore}`
  }

  if (rightResult === "W") {
    return `${match.rightPlayer || "TBD"} won ${rightScore}–${leftScore}`
  }

  if (leftResult === "D" || rightResult === "D") {
  ...
  return `No result yet ${leftScore}–${rightScore}`
}
...
function getRoundBubbleTone(
  ...
  if (normalizedRound === "Qualifiers" || normalizedRound === "Group Stage") {
    return "white"
  }
  ...
  if (normalizedRound === "Finals" || normalizedRound === "Grand Finals") {
    return "gold"
  }
...
export default function TournamentDetailPage({
  ...
  params: Promise<{ game: string; tournament: string }>
}) {
  const { game, tournament } = use(params)
          ...
          return (
            <div key={match.matchId} className="grid gap-2">
              ...
              <div
                ...
                onClick={() =>
                  router.push(
                    `/${game}/tournament/${tournament}/match/${match.matchId}`
                  )
                }
                onKeyDown={(event) => {
                ...
                <div className="flex items-start justify-between gap-6">
                  <div className="min-w-0 flex-1">
                    <h2 className="break-words text-2xl font-bold">
                      {match.leftPlayer || "TBD"} VS{" "}
                      {match.rightPlayer || "TBD"}
                    </h2>
