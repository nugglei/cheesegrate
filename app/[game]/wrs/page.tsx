...
import { getWorldRecordsForCategory } from "../../../lib/leaderboards"
...
function isPurpleWR(map: string, category: string) {
  ...
  if (preset === "skip") {
    return ["Skip IGT", "Skip EGT", "R15 Skip", "R6 Skip"].includes(category)
  }

  if (preset === "rthroskip") {
    return ["Skip IGT", "Skip EGT"].includes(category)
  }

  if (preset === "r15skip") {
    return ["Skip IGT", "Skip EGT", "R15 Skip"].includes(category)
  }

  if (preset === "r6skip") {
    return ["Skip IGT", "Skip EGT", "R6 Skip"].includes(category)
  }
...
export default function WRsPage() {
  ...
  if (game === "swift") {
    return <SwiftWRsPage game={game} />
  }

  return <SpeedRaceWRsPage game={game} />
}

function SpeedRaceWRsPage({ game }: { game: string }) {
  ...
  const wrs = useMemo(
    () => getWorldRecordsForCategory(runs, category),
    [runs, category]
  )
  ...
  return (
    <main style={{ padding: "40px" }}>
      ...
      {loading && <p style={{ color: "#9ca3af" }}>Loading WRs...</p>}
      ...
      >
        {wrs.map((run, index) => {
          ...
          const isPurple = isPurpleWR(run.map, category)
          ...
          return (
            <div
              key={`${run.map}-${run.time}-${index}`}
              style={{
                display: "grid",
                gridTemplateColumns: "340px 110px 180px auto",
            ...
            >
              ...
              >
                ...
                <Link
                  href={`/${game}/lb/${slugifyMap(run.map)}`}
                  style={{
                    paddingLeft: "44px",
                    paddingRight: "44px",
                    fontWeight: 700,
              ...
              >
                ...
                {run.proofs[0] && (
                  <a href={run.proofs[0]} target="_blank">
                    <img
                      src="/video.png"
                      alt="Video"
                      style={{
              ...
              >
                {run.players.length > 1
                  ? run.players.slice(1).map((player, i) => (
                      <span
                        key={`${player}-${i}`}
                        style={{
                          display: "inline-flex",
                      ...
                      >
                        ...
                        {run.proofs[i + 1] && (
                          <a
                            ...
                            target="_blank"
                            style={{
                              marginLeft: hasVeryManyTies ? "2px" : "4px",
                              width: tieVideoSize,
                              height: tieVideoSize,
                              display: "inline-flex",
                              alignItems: "center",
...
function SwiftWRsPage({ game }: { game: string }) {
    ...
    () =>
      leaderboards
        .map((leaderboard) => {
          ...
          const wrTime = entries[0].timeMs
          const tiedEntries = entries.filter(
            (entry) => entry.timeMs === wrTime
          )
          ...
          return {
            ...
            timeMs: wrTime,
            players: tiedEntries.map((entry) => entry.player),
            updatedAt: tiedEntries[0].updatedAt,
          }
        })
        .filter((wr): wr is SwiftWR => wr !== null),
    [leaderboards]
  )

  return (
    <main style={{ padding: "40px", paddingLeft: "330px" }}>
      <h1
        style={{
          fontSize: "36px",
          fontWeight: 700,
      ...
      {loading && (
        <p style={{ color: "#9ca3af" }}>Loading Swift WRs...</p>
      )}
      ...
      >
        {wrs.map((run, index) => {
          const dateOnly = run.updatedAt ? run.updatedAt.split("T")[0] : ""
          ...
          return (
            <div
              ...
              style={{
                display: "grid",
                gridTemplateColumns: "280px 95px 220px",
                borderBottom:
                  index === wrs.length - 1
                    ? "none"
                    : "1px solid rgba(255, 255, 255, 0.1)",
            ...
            >
              ...
              >
                {formatSwiftTime(run.timeMs)}

                {hoveredTime === index && date.tooltip && (
                  <div
                    style={{
                      position: "absolute",
                      top: "34px",
